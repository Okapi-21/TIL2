...
　プリミティブ型とは他の型から派生していない、基本的なデータ型のことを示す。言い換えれば、プリミティブ型とはプログラミング言語で利用することのできる最も基本的なデータ型のことであり、通常、数値、文字列、ブール値など単純な値を表現するときに使用されるものである。ただ、プログラミングを用いて現実世界のものを表現するとき、クラスを定義せず、扱うデータ全てをプリミティブ型で表現してしまうと、現実世界では起こりうることのない数字や文字がプログラムに紛れ込んでしまう可能性がある。これはバクや不具合の要因にもなり、システムが脆弱性を抱えてしまうことを意味する。
　したがって、プログラムで扱うデータに関してはクラスを定義し、その中で定義された変数をプログラムに組み込むことで、現実世界と同様の制約をプログラムに対しても設けることができる。このような設計手法を「ValueObject」と呼び、一意性を持たず、イミュータブル（不変）であるなどの特徴がある。ValueObjectを利用することで、バグや不具合を防ぎ、より堅牢なシステムを設計することができる。
　

＜コメント＞
2025/08/25
　オブジェクト指向プログラミングにおける基礎的な考え方なのかな。プログラムを一つ一つの部品の集合体のように捉えて、その部品の構成要素に関しても自己定義することができる。現実世界のような物質的な制限がなく、完全に論理ので完結する世界だ。プログラムの全体像を見ながら、構築する一つ一つの部品の構成について気を配っていく必要がありそう。
　でも、案外、オブジェクト指向的な考え方は現実世界でも割と気付かないうちに使っていそうだけども。一つの物事に対して、概念化して、重要な部分を抜き取って、再構成しているだけだから。
